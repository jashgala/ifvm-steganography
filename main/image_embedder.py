...
def embed(cover, secret, occ, saveLocation = "output.avi"):
	...
	secret = secret Image Path/Location'''
	...
	secret_image_text_file_path = "secret_image_file_154861.txt"
	with open(secret, "rb") as imageFile:
		img_data = base64.b64encode(imageFile.read())

	with open(secret_image_text_file_path, "w") as txt:
		txt.write(img_data)
	...
	blocks = ifvm.generateTextBlocks(secret_image_text_file_path, byteCapacity)
	os.remove(secret_image_text_file_path)
