...
def extract(cover, key):
	...
	audio_txt_location = 'outfile.' + indexData[0]
	audio_location = 'extractedAudio.' + indexData[1]

	f = open(audio_txt_location, 'w')
	...
	with open(audio_txt_location,"rb") as res:
		str = base64.b64decode(res.read())
	fh = open(audio_location, "wb")
	fh.write(str)
	fh.close()
	os.remove(audio_txt_location)
