...
def embed(cover, secret, occ, saveLocation = "output.avi"):
	...
	secret = secret Audio Path/Location'''
	...
	secret_audio_text_file_path = "secret_audio_file_154861.txt"
	with open(secret, "rb") as audioFile:
		aud_data = base64.b64encode(audioFile.read())

	with open(secret_audio_text_file_path, "w") as txt:
		txt.write(aud_data)
	...
	blocks = ifvm.generateTextBlocks(secret_audio_text_file_path, byteCapacity)
	os.remove(secret_audio_text_file_path)
