...
def embed(cover, secret, occ, saveLocation = "output.avi"):
	...
	secret_video_text_file_path = "secret_video_file_154861.txt"
	with open(secret, "rb") as videoFile:
		vid_data = base64.b64encode(videoFile.read())

	with open(secret_video_text_file_path, "w") as txt:
		txt.write(vid_data)
	...
	blocks = ifvm.generateTextBlocks(secret_video_text_file_path, byteCapacity)
	os.remove(secret_video_text_file_path)
